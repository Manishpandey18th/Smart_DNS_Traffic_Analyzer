...
# EXPLAIN: Defines the GUI class for the Smart DNS Traffic Analyzer
class SmartDNSAnalyzerGUI:
    # EXPLAIN: Initializes the GUI window and core variables
    def __init__(self, root):
        self.root = root
        self.root.title("Smart DNS Traffic Analyzer")
        self.root.geometry("800x600")
        self.running = False  # EXPLAIN: Tracks if monitoring is active
        self.stop_event = threading.Event()  # EXPLAIN: Signals the sniffing thread to stop
        self.analyzer = SmartDNSAnalyzer()  # EXPLAIN: Core analyzer instance
        self.filter_status = {"normal": True, "suspicious": True, "spoofing": True}  # EXPLAIN: Filter for packet types
        self.packet_log = []  # EXPLAIN: Stores packet data for display and reporting

        self.create_widgets()  # EXPLAIN: Sets up the GUI elements

    # EXPLAIN: Creates the GUI layout with thresholds, filters, buttons, and output display
    def create_widgets(self):
        ...
        self.entropy_var = tk.DoubleVar(value=3.8)  # EXPLAIN: Default entropy threshold
        ...
        self.length_var = tk.IntVar(value=35)  # EXPLAIN: Default query length threshold
        ...
        self.ttl_var = tk.IntVar(value=10)  # EXPLAIN: Default TTL threshold
        ...
        self.freq_var = tk.IntVar(value=5)  # EXPLAIN: Default frequency threshold
        ...
        # EXPLAIN: Configures color tags for different packet statuses
    ...
    # EXPLAIN: Updates filter settings and refreshes the display
    ...
    # EXPLAIN: Starts packet monitoring in a separate thread
    ...
    # EXPLAIN: Stops packet monitoring and updates button states
    ...
    # EXPLAIN: Sniffs DNS packets (UDP port 53) and processes them
    ...
    # EXPLAIN: Plays an alert sound for suspicious or spoofing packets
    def play_alert(self, tag):
        if tag == "suspicious":
            winsound.Beep(1000, 200)  # EXPLAIN: 1000 Hz for 200 ms
        elif tag == "spoofing":
            winsound.Beep(1500, 300)  # EXPLAIN: 1500 Hz for 300 ms

    # EXPLAIN: Analyzes a packet, determines its status, and logs/displays it
    ...
    # EXPLAIN: Displays a single packet in the GUI with formatted details
    ...
    # EXPLAIN: Refreshes the entire display based on current filter settings
    def refresh_display(self):
        """Refresh the entire display based on current filter settings."""
        self.output_text.delete(1.0, tk.END)  # EXPLAIN: Clears current display
        self.output_text.insert(tk.END, "Smart DNS Traffic Analyzer - Ready to monitor\n", "header")
        self.output_text.insert(tk.END, "Normal (White) | Suspicious (Yellow) | Spoofing (Red)\n", "header")
    ...
    # EXPLAIN: Generates a text file report with packet summary and details
...
# EXPLAIN: Defines the core DNS analysis logic
class SmartDNSAnalyzer:
    # EXPLAIN: Initializes the analyzer with thresholds, model, and lists
    def __init__(self, log_file="dns_threats.log", entropy_threshold=3.5, length_threshold=35, ttl_threshold=10, freq_threshold=5):
        logging.basicConfig(filename=log_file, level=logging.INFO, format="%(asctime)s - %(message)s")
        self.logger = logging.getLogger()
        self.model = self.train_model()  # EXPLAIN: Trains ML model for classification
        self.entropy_threshold = entropy_threshold
        self.length_threshold = length_threshold
        self.ttl_threshold = ttl_threshold
        self.freq_threshold = freq_threshold
        self.query_counts = defaultdict(list)  # EXPLAIN: Tracks query frequency
        self.allowlist = self.load_list("allowlist.txt")  # EXPLAIN: Whitelist of trusted domains
        self.blocklist = self.load_list("blocklist.txt")  # EXPLAIN: Blacklist of blocked domains

    # EXPLAIN: Loads allowlist or blocklist from a file
    ...
    # EXPLAIN: Calculates Shannon entropy of a string (query)
    ...
    # EXPLAIN: Extracts features from a DNS packet
    ...
    # EXPLAIN: Trains a Random Forest model with hardcoded data
    ...
    # EXPLAIN: Predicts if a query is tunneling using the ML model
    ...
    # EXPLAIN: Detects spoofing based on low TTL in responses
    def detect_spoofing(self, features):
        return features["ttl"] is not None and features["ttl"] < self.ttl_threshold

    # EXPLAIN: Tracks query frequency within a 10-second window
...
# EXPLAIN: Main function to start the GUI application
def main():
    root = tk.Tk()
